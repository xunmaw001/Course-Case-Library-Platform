            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">案例收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="anliName" id="anliName" placeholder="案例名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">制作主题</label>
                              <div class="layui-input-inline">
                                  <select name="anliZhutiTypes" id="anliZhutiTypes" lay-filter="anliZhutiTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in anliZhutiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="anliTypes" id="anliTypes" lay-filter="anliTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in anliTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">案例形式</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="anliXingshi" id="anliXingshi" placeholder="案例形式" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../anli/detail.jsp?id='+item.anliId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.anliPhoto?item.anliPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../anli/detail.jsp?id='+item.anliId)"><span>{{item.anliName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.anliNewMoney"><span style="color: red;">{{item.anliNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteAnliCollection(item.id)">删除 {{item.anliName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 anliCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            anliCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteAnliCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('anliCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#anliName').val() != null && jquery('#anliName').val() != "" && jquery('#anliName').val() != "null" ) {
                param['anliName'] = jquery('#anliName').val();
            }
                     
            var anliZhutiTypes = $("#anliZhutiTypes").find("option:selected").val();
            if (anliZhutiTypes != null && anliZhutiTypes != "" && anliZhutiTypes != "null") {
                            param['anliZhutiTypes'] = anliZhutiTypes;
                        }
                     
            var anliTypes = $("#anliTypes").find("option:selected").val();
            if (anliTypes != null && anliTypes != "" && anliTypes != "null") {
                            param['anliTypes'] = anliTypes;
                        }
                     
            if (jquery('#anliXingshi').val() != null && jquery('#anliXingshi').val() != "" && jquery('#anliXingshi').val() != "null" ) {
                param['anliXingshi'] = jquery('#anliXingshi').val();
            }
                                                                                
            // 当前表
                                                    // 获取列表数据
            http.request('anliCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function anliZhutiTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=anli_zhuti_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.anliZhutiTypesList = res.data.list;
               }
           });
       }
       ...
       function anliTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=anli_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.anliTypesList = res.data.list;
               }
           });
       }
